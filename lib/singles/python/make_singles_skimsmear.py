...
ANALYZEDIR = '~/share/WATCHMAN/PMTActivityStudy/2500/25pct/2500_25pct_accidentals.root'
#######NAME OF OUTPUT FILE##########
...
if __name__ == '__main__':
    ...
    while (not FileDepleted) and (entrynum < 10000000):
        ...
        is_fiducial_rf[0] = eu.isFiducial(FV_RADIUS, FV_HEIGHT, bkgtree.x, bkgtree.y, 
                bkgtree.z)
        pe_rf[0] = bkgtree.pe * pd.RandShootSmear(SMEARPCT,1)
        n9_rf[0] = bkgtree.n9 * pd.RandShootSmear(SMEARPCT,1)

        nhit_rf[0] = bkgtree.nhit
        good_pos_rf[0] = bkgtree.good_pos * pd.RandShootSmear(SMEARPCT,1)
        u_rf[0] = bkgtree.u
        v_rf[0] = bkgtree.v
        w_rf[0] = bkgtree.w
        x_rf[0] = bkgtree.x
        ...
        z_rf[0] = bkgtree.z
        good_dir_rf[0] = bkgtree.good_dir * pd.RandShootSmear(SMEARPCT,1)
        closestPMT_rf[0] = bkgtree.closestPMT * pd.RandShootSmear(SMEARPCT,1)
        t_root.Fill()
        entrynum+=1
    f_root.cd()
    t_root.Write()
