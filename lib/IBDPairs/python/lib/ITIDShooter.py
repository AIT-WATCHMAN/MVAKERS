...
def MakeTimeHist(rootfiles,timethresh):
    ...
    for rf in rootfiles:
        datchain.Add(rf)
    print("EVS IN CHAIN FOR TIMEHIST: " + str(datchain.GetEntries())) 
    FileDepleted = False
    entrynum=0
    while (not FileDepleted) and (entrynum < 10000000):
        datchain.GetEntry(entrynum)
        if 0 < datchain.dt < timethresh:
...
def MakeTimeFit_landau(timehist,timethresh):
    ...
    print("ENTERING LANDAU TIME FIT")
    TimeFit = ROOT.TF1('TimeFit', landau, 1,
            float(timethresh), 3)
    TimeFit.SetParameter(0, 10000.0)
    #TimeFit.SetParLimits(0, 0, )
    TimeFit.SetParameter(1, 10.0)
    #TimeFit.SetParLimits(1, 0, 1E7)
    TimeFit.SetParameter(2, 10)
...
def MakeTimeFit_wald(timehist,timethresh):
    #Function takes in a time histogram and fits a polynomial to it. Returns
    #The polynomial function for random firing of values from
